...
try:
    ...
    if price_keyword != "모두 보기":
        if price_keyword == "Free":
            filtered_df = filtered_df[filtered_df["price"].str.contains("Free", na=False)]
        elif "₩" in price_keyword:
            threshold_str = ''.join(c for c in price_keyword if c.isdigit())
            try:
                threshold = int(threshold_str)
                def price_condition(x):
                    p = extract_price(x)
                    if p is None:
                        return False
                    return p <= threshold if "이하" in price_keyword else p > threshold
                filtered_df = filtered_df[filtered_df["price"].apply(price_condition)]
            except ValueError:
                st.warning("가격 필터 숫자 변환 중 오류가 발생했습니다.")
